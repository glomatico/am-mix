...
const { getVersion } = useConfigFile();
...
<template>
  <v-card variant="text" :disabled="updateResult.loadingState === LoadingState.LOADING">
    <v-row>
      <SettingsSection title-key="settings.sections.version" />
      ...
      <v-col cols="12" md="6">
        ...
        <p class="ma-0">{{ $t('settings.version', { version, }) }}</p>
      </v-col>
      ...
      <v-col cols="12" md="6">
        <FileDialogField v-model:file-path="newJsonConfig.wvd" :label="$t('settings.baseInterface.wvd')" filter="*.wvd"
          nullable />
      </v-col>
      ...
      <v-col cols="12" md="6">
        <v-radio-group v-model="newJsonConfig.download_mode" :label="$t('settings.baseDownloader.downloadMode.title')">
      ...
      <v-col cols="12" md="6">
        <v-combobox variant="outlined" rounded v-model="newJsonConfig.song_codec" multiple chips clearable hide-details
          :label="$t('settings.baseInterface.songCodec')" />
      </v-col>
      ...
      <v-col cols="12" md="6">
        <v-switch v-model="newJsonConfig.synced_lyrics_only" :label="$t('settings.baseInterface.syncedLyricsOnly')" />
      </v-col>
